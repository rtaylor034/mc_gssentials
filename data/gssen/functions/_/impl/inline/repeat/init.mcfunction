...
data modify storage gssen:var repeat.scopes append from storage gssen:in repeat
data remove storage gssen:in repeat

$scoreboard players set *repeat.iter.$(scope) gssen_var 0
function gssen:_/impl/inline/repeat/command with storage gssen:var repeat.scopes[-1]
